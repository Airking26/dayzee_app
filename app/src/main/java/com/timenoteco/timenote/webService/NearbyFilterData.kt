...
class NearbyFilterData(context: Context) {
    ...
    private var nearbyFilterModel: NearbyRequestBody? =
        Gson().fromJson<NearbyRequestBody>(prefs.getString("nearby", Gson().toJson(NearbyRequestBody(Location(0.0, 0.0, Address("","", "","")), 1, listOf(), SimpleDateFormat(ISO, Locale.getDefault()).format(System.currentTimeMillis()), Price(0, ""), 2))), type)
